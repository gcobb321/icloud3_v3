        ...
        for sensor_def in new_sensors_list:
            ...
            for from_zone in conf_device[CONF_TRACK_FROM_ZONES]:
                ...
                if sensor_zone in devicename_from_zone_sensors:
                    ...
                    Sensor = SensorTfzFrom(devicename, sensor_def, conf_device, from_zone)
...
#-------------------------------------------------------------------------------------------
    def _get_extra_attributes(self, sensor_def):
        ...
        attrs = self._get_sensor_definition(sensor_def, SENSOR_ATTRS)
        extra_attrs = {}
        for attr in attrs:
            attr_sensor_value = self._get_sensor_value(attr)

            # Interval sensor may end in -5s or -15s. if so, strip it off attr value
            if (attr == INTERVAL and attr_sensor_value.endswith('s')):
                datetime_parts = attr_sensor_value.split('-')
                attr_sensor_value = f"{datetime_parts[0]}-{datetime_parts[1]}-{datetime_parts[2]}"

            extra_attrs[attr] = attr_sensor_value
...
#-------------------------------------------------------------------------------------------
    def _get_sensor_value(self, sensor):
        ...
        try:
            if self.Device is None:
                sensor_value = self.default_value

            # elif self.from_zone and self.Device.DeviceFmZones_by_zone is None:
            elif self.from_zone and self.DeviceFmZone is None:
                sensor_value = self.default_value

            elif self.from_zone is None:
                sensor_value = self.Device.sensors.get(sensor, None)

            elif self.DeviceFmZone:
                # Strip off zone to get the actual tfz dictionary item
                tfz_sensor = sensor[:sensor.rfind("_")]
                sensor_value = self.DeviceFmZone.sensors.get(tfz_sensor, None)

            if sensor_value is None or sensor_value == NOT_SET:
                sensor_value = self.default_value

            elif type(sensor_value) is str and sensor_value.strip() == '':
                sensor_value = self.default_value
...
#-------------------------------------------------------------------------------------------
    def _get_sensor_um(self, sensor):
        ...
        try:
            ...
            # if self.from_zone and self.Device.DeviceFmZones_by_zone is None:
            ...
            # elif self.from_zone in self.Device.DeviceFmZones_by_zone:
            #     sensor_um = self.Device.DeviceFmZones_by_zone[self.from_zone].sensors_um.get(sensor, None)
...
#<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
class SensorTfzFrom(iCloud3SensorBase):
    '''  Sensor for displaying the device zone time/distance items '''

    @property
    def native_value(self):
        self._attr_unit_of_measurement = None
        sensor_value = self.DeviceFmZone.from_zone_display_as if self.DeviceFmZone else self.from_zone
...
class SensorInfo(iCloud3SensorBase):
    '''
        Sensor for handling info sensor messages.
            ...
            2.  This will broadcast a message to all 'info' sensors using the
...
class SensorTimestamp(iCloud3SensorBase):
    ...
    @property
    def extra_state_attributes(self):
        if instr(self.sensor, 'datetime'):
            return None

        datetime_value = self._get_sensor_value(f"{self.sensor}_datetime")
        extra_attrs = {}
        extra_attrs['datetime'] = datetime_value
        return extra_attrs
...
class iCloud3SupportiCloud3SensorBase(SensorEntity):
    ...
    def __init__(self, fname, entity_name):
        ...
        self.entity_id         = f"sensor.{self.entity_name}"
        self._unsub_dispatcher = None
        log_info_msg(f'{self.fname=} {self.entity_name=} {self.entity_id=}')
