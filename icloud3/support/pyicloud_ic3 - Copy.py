...
# FILTER_DATA_DICT_ITEMS = ['devices', 'userInfo', 'dsid', 'dsInfo', 'webservices', 'content',
#         'locations', 'followers', 'following', 'contactDetails',]
# FILTER_DATA_LIST_ITEMS = ['content', 'followers', 'following', 'contactDetails']

# def log_pyicloud_rawdata(title, rawdata):
#     '''
#     Filter the pyicloud raw data
#     '''
...
#     filtered_data = {}
#     rawdata_data = rawdata if 'data' in rawdata else {'data': rawdata}

#     for data_dict_item in FILTER_DATA_DICT_ITEMS:
#         filter_results = _filter_data_dict(rawdata_data['data'], data_dict_item)
#         if filter_results:
#             filtered_data[data_dict_item] = filter_results

#     for data_list_items in FILTER_DATA_LIST_ITEMS:
#         filter_results = _filter_data_list(rawdata_data['data'], data_list_items)
#         if filter_results:
#             filtered_data[data_list_items] = filter_results

#     LOGGER.info(f"{'─'*8} {title.upper()} {'─'*8}")

#     if filtered_data == {}:
#         LOGGER.info(rawdata_data)
#     else:
...
#     filtered_data_list = []
#     try:
#         for data_list_item in rawdata_data[data_list_items]:
...
class PyiCloudSession(Session):
    ...
    def request(self, method, url, **kwargs):  # pylint: disable=arguments-differ
        ...
        if (not response.ok and (content_type not in json_mimetypes
            ...
            if has_retried == False and response.status_code in [421, 450, 500]:
                ...
                return self.request(method, url, **kwargs)
            _traceha(f"195 {response.reason=} {response.status_code=} {has_retried=} {kwargs=}")
    ...
    def _raise_error(self, code, reason):
        ...
        if code in ("ZONE_NOT_FOUND", "AUTHENTICATION_FAILED"):
            ...
            LOGGER.info("233 Authentication needed for Account (421)")
            return
        ...
        _traceha(f"249 {api_error=} {reason=} {code=}")
        LOGGER.error(f"249 {api_error}")
        raise api_error
