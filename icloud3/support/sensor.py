...
from ..const             import (NOT_SET, TITLE, DATETIME_ZERO, DIST, PAUSED_CAPS, HIGH_INTEGER,
                                ...
                                CONF_UNIT_OF_MEASUREMENT, FRIENDLY_NAME, ICON, BADGE,
...
def x_async_setup_platform(hass, config, async_add_entities,
                               discovery_info=None):
    _trace('@@@@@ sensor.py, 48-async_setup_platform')
    """Set up the file sensor."""
    # await async_setup_reload_service(hass, DOMAIN, PLATFORM)
    # _trace('@@@@@ sensor.py, 54-Before async_add_entities')
    # async_add_entities([iCloud3Sensor(config)], True)
    # _trace('@@@@@ sensor.py, 54-After async_add_entities')

# async def async_setup_entry(hass, config_entry, async_add_devices):
# async def create_icloud3_sensors_for_devices():
def create_icloud3_sensors_for_devices():
    _trace('@@@@@ sensor.py, 59-async_setup_entity')
    ...
    for devicename, Device in Gb.Devices_by_devicename.items():
        try:
            old_DeviceSensors_by_sensor = Device.Sensors_by_sensor.copy()
        except:
            old_DeviceSensors_by_sensor = {}

        for sensor in Gb.Sensors.sensors_custom_list:
            if sensor in old_DeviceSensors_by_sensor:
                DeviceSensor = old_DeviceSensors_by_sensor[sensor]
                DeviceSensor.__init__(Device, sensor)
            else:
                DeviceSensor = iCloud3Sensor(Device, sensor)
                HA_DeviceSensors.append(DeviceSensor)

            Device.Sensors_by_sensor[sensor] = DeviceSensor

    _traceha(f"{HA_DeviceSensors}")
    # async_add_devices(HA_DeviceSensors, True)
    _trace('@@@@@ sensor.py, 69-After async_add_entities')


class iCloud3Sensor(Entity):
    """Implementation of a file sensor."""

    def __init__(self, Device, sensor):
        self.entity_id  = f'sensor._new_{sensor}'
        self._name      = (f"{Device.fname}{SENSOR_FNAME[sensor]}") if sensor in SENSOR_FNAME else sensor.title()
        self._attr_icon = SENSOR_ICON[sensor] if sensor in SENSOR_ICON else None
        self._attr_entity_picture       = Device.picture if Device.picture else None
        self._attr_unit_of_measurement  = self._set_unit_of_measurement(sensor)
        self._state                     = '___'
        self._attr_extra_state_attributes = {}
        _traceha.info(self.__dict__)
    ...
    def _set_unit_of_measurement(sensor):
        format_type = SENSOR_FORMAT[sensor]
        um = None
...
class UpdateiCloud3Sensors(object):

    def __init__(self):
        # self.hass                = hass
        # self.create_sensor_ids   = create_sensor_ids
        # self.exclude_sensor_ids  = exclude_sensor_ids
        self.sensors_custom_list = [k for k, v in Gb.conf_sensors.items() if v is True]
...
###############################################################################
#
#   SET UP THE DEVICE'S SENSORS
#
###############################################################################
    # async def async_setup_entry(hass, config_entry, async_add_devices):
    async def create_icloud3_sensors_for_devices(self):
        _trace('@@@@@ sensor.py, 59-async_setup_entity')
        """Add file sensor entities from configuration flow."""
        ...
        # # _LOGGER.info(f"@@@@@@@@@@@@@@@@@ 66 platform={PLATFORM} domain-{DOMAIN} Platform_schema-{PLATFORM_SCHEMA} Sensor_schema-{SENSOR_SCHEMA} ")
        # _LOGGER.info("setup entity-config_entry_data=%s",result)
        # await async_setup_reload_service(hass, DOMAIN, PLATFORM)
        # _LOGGER.warning('@@@@@ sensor.py, 69-Before async_add_entities')
        # async_add_devices([iCloud3Sensor(result)], True)
        if Gb.Devices_by_devicename == {}:
            return
    ...
    def update_device_sensors(self, Device, attrs:dict, DeviceFmZone=None):
        ...
        try:
            ...
            for attr_name in SENSOR_DEVICE_ATTRS:
                ...
                sensor_attrs = {}
                format_type  = SENSOR_FORMAT.get(attr_name, '')

                if Gb.start_icloud3_inprocess_flag and attr_name != INFO:
                    if attr_name == LAST_UPDATE_TIME:
                        state_value = 'Never'
                    elif format_type == DIST:
                        state_value = 0.0
                    else:
