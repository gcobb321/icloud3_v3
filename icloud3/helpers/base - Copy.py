...
FILTER_DATA_DICTS = ['data', 'devices', 'userInfo', 'dsid', 'dsInfo', 'webservices', ]
        # 'content',  'locations', 'followers', 'following', 'contactDetails',]
FILTER_DATA_LISTS = ['content', 'followers', 'following', 'contactDetails']
FILTER_FIELDS = [
        ...
        'locations', 'followers', 'following', 'contactDetails',
        'data', 'devices', 'userInfo', 'dsid', 'dsInfo', 'webservices', 'content',
        'locations', 'followers', 'following', 'contactDetails', ]
...
def log_rawdata(title, rawdata):
    ...
    rawdata_data = {}
    if 'data' in rawdata:
        # rawdata_data = rawdata
        # filtered_data['data'] = _filter_data_dict(rawdata_data['data'], 'data')
        rawdata_data['filter'] = rawdata['data']#_filter_data_dict(rawdata['data'], 'data')
    else:
        rawdata_data['filter'] = rawdata
    # _traceha(f"{rawdata_data=}")
        # log_info_msg(rawdata_data)
        # return
    # _traceha(f"{rawdata=}")
    ...
    for data_dict in FILTER_DATA_DICTS:
        ...
        if filter_results:
            filtered_data[data_dict] = filter_results

    for data_list in FILTER_DATA_LISTS:
        filter_results = _filter_data_list(rawdata_data['filter'], data_list)
        if filter_results:
            filtered_data[data_list] = filter_results
            # _traceha(f"{data_list=} {filter_results=}")

    if filtered_data:
        log_info_msg(f"filtered-{filtered_data}")
    else:
        _traceha(f"**********368 {rawdata_data=}")
        _traceha(f"*********368 *{rawdata_data['filter']=}")
        # filtered_results = _filter_data_list(rawdata_data, 'filter')
        try:
            filtered_results = {k: v for k, v in rawdata_data['filter'].items()
                                        if k in FILTER_FIELDS}
            log_info_msg(f"374 rawfiltered-{filtered_results}")
            log_info_msg(f"375 rawraw-{rawdata_data}")
        except:
            pass
...
def _filter_data_dict(rawdata_data, data_dict_items):
    try:
        _traceha(f"**********384 {data_dict_items=}")
        filtered_item = {k: v for k, v in rawdata_data[data_dict_items].items()
                                    if k in FILTER_FIELDS}

        if 'id' in filtered_item and len(filtered_item['id']) > 10:
            filtered_item['id'] = f"{filtered_item['id'][:10]}..."
...
#--------------------------------------------------------------------
def _filter_data_list(rawdata_data, data_list_items):
    # filtered_data_list = []
    try:
        _traceha(f"**********403 {data_list_items=} {rawdata_data=}")
        filtered_item = {k: v for k, v in rawdata_data.items()
                                    if k in FILTER_FIELDS}
        if 'id' in filtered_item and len(filtered_item['id']) > 10:
            filtered_item['id'] = f"{filtered_item['id'][:10]}..."

        if 'location' in filtered_item and filtered_item['location']:
            filtered_item['location'] = {k: v for k, v in filtered_item['location'].items()
                                                    if k in FILTER_FIELDS}
    except:
        filtered_item = {}
        pass

    _traceha(f"**********414 {filtered_item=}")
    return filtered_item
        ...
        # _traceha(f"{rawdata_data=}")
        for data_list_item in rawdata_data.get(data_list_items):
            if data_list_item is None:
                continue

            _traceha(f"**********403 {data_list_items=} {data_list_item=}")
            # _traceha(f"{data_list_item=}")
            filtered_item = {k: v for k, v in data_list_item.items()
                                    if k in FILTER_FIELDS}
            if 'id' in filtered_item and len(filtered_item['id']) > 10:
                filtered_item['id'] = f"{filtered_item['id'][:10]}..."
            ...
            if 'location' in filtered_item and filtered_item['location']:
                filtered_item['location'] = {k: v for k, v in filtered_item['location'].items()
                                                    if k in FILTER_FIELDS}
            # _traceha(f"{filtered_item=}")
            filtered_data_list.append(filtered_item)
            # _traceha(f"{filtered_data_list=}")
...
def xlog_rawdata(title, raw_data, log_rawdata=False):
    ...
    if ((Gb.log_rawdata_flag or log_rawdata) and Gb.log_rawdata_flag_unfiltered):
        log_info_msg(f"{'─'*20} {display_title} ─{'─'*20}")
        log_info_msg(f"{raw_data}")
        log_info_msg("-"*70)
        return

    filtered_data = {k: v for k, v in raw_data.items()
                                if k in LOG_RAWDATA_FIELDS}
