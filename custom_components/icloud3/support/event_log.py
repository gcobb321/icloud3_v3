...
from ..const_general     import (DOT, HHMMSS_ZERO, HIGH_INTEGER, STATIONARY, CRLF, CRLF_DOT, CRLF_CHK,
                                EVLOG_ERROR,
...
from ..helpers.base      import instr, is_statzone, log_exception, _trace, _traceha, log_info_msg
...
#--------------------------------------------------------------------
    ...
    # def x_export_ic3_event_log_reformat_recds(self, devicename, recd_str):
    ...
    #     recd_str = recd_str.replace(devicename, "")

    #     for from_text in self.display_text_as:
    #         recd_str = recd_str.replace(from_text, self.display_text_as.get(from_text))[2:]
    #     _traceha(f"{recd_str}")
