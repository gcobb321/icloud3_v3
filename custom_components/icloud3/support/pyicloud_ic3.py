...
from ..const_general     import (FAMSHR, FMF, FMF_FNAME, FAMSHR_FNAME, HHMMSS_ZERO, )
from ..const_attrs       import (NAME, ID, LOCATION, TIMESTAMP, ICLOUD_TIMESTAMP, )
from ..helpers.time      import (time_now_secs, secs_to_time, msecs_to_time, )
from ..helpers.base      import (instr, post_event, post_monitor_msg, _trace, _traceha)
...
class PyiCloudSession(Session):
    ...
    def request(self, method, url, **kwargs):  # pylint: disable=arguments-differ
        ...
        LOGGER.debug(f"REQUEST -- {secs_to_time(time_now_secs())}, {method}, {url}, {kwargs.get('data', '')}")
        ...
        # LOGGER.debug(f"RESPONSE -- {response}, StatusCode-{response.status_code}, okStatus-{response.ok}, hasRetried-{has_retried}")
    ...
    def refresh_client(self, _device_id=None, _with_family=None, refreshing_poor_loc_flag=False):
        ...
        for device_info in self.response["content"]:
            ...
            if instr(device_name, u'\xa0'):
                device_name       = device_name.replace(u'\xa0', u' ')
            ...
            if (LOCATION not in device_info
                ...
                if device_id not in self.devices_without_location_data:
                    ...
                    monitor_msg = (f"NOT ADDED FamShr PyiCloud_DevData-{device_id[:8]}, "
                                    f"{device_name}, No Location Data")
            ...
            if device_id in Gb.PyiCloud_DevData_by_device_id:
                if  device_id in Gb.Devices_by_icloud_device_id:
                    ...
                    monitor_msg = (f"UPDATED, FamShr, PyiCloud_DevData, {device_id[:8]}, "
                                    f"{device_name}, {device_info=}")
                ...
                post_monitor_msg(f"ADDED FamShr PyiCloud_DevData-{device_id[:8]}, {device_name}")
...
#----------------------------------------------------------------------------
    def refresh_client(self, refreshing_poor_loc_flag=False):
        ...
        try:
            ...
            for device_info in self.response['locations']:
                device_id   = device_info[ID]
                device_name = ''
                # if device_id.startswith('NDM0NTU2'): device_info[LOCATION] = {}
                # if device_id.startswith('MTg2Mjk3'): device_info[LOCATION] = {}

                # Do not add the device if no location data
                if (LOCATION not in device_info
                        or device_info[LOCATION] == {}
                        or device_info[LOCATION] is None):
                    if device_id not in self.devices_without_location_data:
                        self.devices_without_location_data.append(device_id)
                        monitor_msg = (f"NOT ADDED FmF PyiCloud_DevData-{device_id[:8]}, "
                                       f"No Location Data")
                        post_monitor_msg(monitor_msg)
                    continue
                ...
                if device_id in Gb.PyiCloud_DevData_by_device_id:
                    if device_id in Gb.Devices_by_icloud_device_id:
                        ...
                        monitor_msg = (f"UPDATED FmF PyiCloud_DevData, {device_id[:8]}, "
                                        f"{PyiCloud_DevData.location=}")
                        LOGGER.debug(monitor_msg)
                        # post_monitor_msg(f"UPDATED FmF PyiCloud_DevData, {device_id[:8]}")
                else:
                    _PyiCloud_DevData = PyiCloud_DevData(
                        device_id, device_info, self.Session, self.params,
                        FmpiOrFmfSvcMgr=self, device_name=device_name,
                    ...
                    LOGGER.debug(f"ADDED FmF PyiCloud_DevData, {device_id[:8]}, {PyiCloud_DevData.location=}")
                    post_monitor_msg(f"ADDED FmF PyiCloud_DevData-{device_id[:8]}")
...
class PyiCloud_DevData(object):
    '''
    PyiCloid_Device stores all the device data for Family Sharing and Find-my-Friends
