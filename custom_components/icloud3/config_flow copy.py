...
#-------------------------------------------------------------------------------------------
    def _validate_other_parms(self, user_input):
        ...
        user_input = self._minutes_to_hhmmss(user_input)
...
#-------------------------------------------------------------------------------------------
    def _hhmmss_to_duration(self, hhmmss_field_name, conf_group=None):
        ...
        return duration
#-------------------------------------------------------------------------------------------
    def _minutes_to_hhmmss(self, user_input):
        """ Reformat the duration parameters from {'hours', 'minutes' ,'seconds'} to hhh:mm:ss """

        new_user_input = user_input.copy()
        for pname, pvalue in user_input.items():
            if pname in CONF_PARAMETER_TIME_STR:
                new_pvalue = f"00:{int(pvalue):02}:00"
            # if type(pvalue) is dict and 'hours' in pvalue:
                # new_pvalue = f"{int(pvalue['hours']):02}:{int(pvalue['minutes']):02}:{int(pvalue['seconds']):02}"
                new_user_input[pname] = new_pvalue
...
#-------------------------------------------------------------------------------------------
    def _hhmmss_to_minutes(self, hhmmss_field_name, conf_group=None):
        """ Reformat hh:mm:ss to {'hours', 'minutes' ,'seconds'} """
        ...
        hhmmss_parts = hhmmss.split(':')
        duration = int(hhmmss_parts[0])*60 + int(hhmmss_parts[1])
        ...
        #------------------------------------------------------------------------
            ...
            schema = vol.Schema({
                ...
                vol.Required(CONF_INZONE_INTERVAL,
                            default=self._hhmmss_to_minutes(CONF_INZONE_INTERVAL, CONF_DEVICES)):
        ...
        #------------------------------------------------------------------------
            ...
            schema = vol.Schema({
                ...
                vol.Required(CONF_OLD_LOCATION_THRESHOLD,
                            default=self._hhmmss_to_minutes(CONF_OLD_LOCATION_THRESHOLD)):
                            selector.NumberSelector(selector.NumberSelectorConfig(
                ...
                vol.Required(CONF_OLD_LOCATION_ADJUSTMENT,
                            default=self._hhmmss_to_minutes(CONF_OLD_LOCATION_ADJUSTMENT)):
                            selector.NumberSelector(selector.NumberSelectorConfig(
                ...
                vol.Required(CONF_MAX_INTERVAL,
                            default=self._hhmmss_to_minutes(CONF_MAX_INTERVAL)):
                            selector.NumberSelector(selector.NumberSelectorConfig(
                ...
                vol.Required(CONF_OFFLINE_INTERVAL,
                            default=self._hhmmss_to_minutes(CONF_OFFLINE_INTERVAL)):
                            selector.NumberSelector(selector.NumberSelectorConfig(
                                min=1, max=240, step=1, unit_of_measurement='minutes')),
        ...
        #------------------------------------------------------------------------
            ...
            schema = vol.Schema({
                vol.Optional(CONF_WAZE_USED,
                            default=Gb.conf_general[CONF_WAZE_USED]):  cv.boolean,
                            # selector.BooleanSelector(),
                ...
                vol.Required(CONF_WAZE_HISTORY_DATABASE_USED,
                            default=Gb.conf_general[CONF_WAZE_HISTORY_DATABASE_USED]):  cv.boolean,
                            # selector.BooleanSelector(),
                vol.Required(CONF_WAZE_HISTORY_MAX_DISTANCE,
                            default=Gb.conf_general[CONF_WAZE_HISTORY_MAX_DISTANCE]):
                            selector.NumberSelector(selector.NumberSelectorConfig(
                                min=0, max=1000, unit_of_measurement='km')),
        ...
        #------------------------------------------------------------------------
            ...
            schema = vol.Schema({
                ...
                vol.Required(CONF_STAT_ZONE_STILL_TIME,
                            default=self._hhmmss_to_minutes(CONF_STAT_ZONE_STILL_TIME)):
                            selector.NumberSelector(selector.NumberSelectorConfig(
                ...
                vol.Required(CONF_STAT_ZONE_INZONE_INTERVAL,
                            default=self._hhmmss_to_minutes(CONF_STAT_ZONE_INZONE_INTERVAL)):
                            selector.NumberSelector(selector.NumberSelectorConfig(
                ...
                vol.Optional('track_from_zone_header',
                            default=track_from_zone_home): cv.boolean,
                vol.Required(CONF_TRACK_FROM_BASE_ZONE,
