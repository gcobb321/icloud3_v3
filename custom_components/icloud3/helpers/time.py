...
from ..const_general    import (UNKNOWN, HIGH_INTEGER, HHMMSS_ZERO, DATETIME_ZERO, DATETIME_FORMAT,
                                NOT_HOME, NOT_SET, PAUSED, PAUSED_CAPS, ICLOUD3_ERROR_MSG, EVLOG_DEBUG,
                                IOSAPP, ICLOUD, EVLOG_NOTICE,  CRLF_DOT, CRLF,)
...
def secs_to_time(timestamp_secs, hour_24=False):
    ...
    if timestamp_secs is None or timestamp_secs == 0 or timestamp_secs == HIGH_INTEGER:
        ...
        return  (f"{time.strftime(time_format, t_struct).lstrip('0')}")     #"={timestamp4(timestamp_secs)}")
