...
from .const_general     import (NOT_SET, HOME, NOT_HOME, ERROR, VALID_DATA, AWAY_FROM, PAUSED,
                                ...
                                NEAR_ZONE, REGION_EXITED, IOSAPP_FNAME,
                                RETRY_INTERVAL_RANGE_1, RETRY_INTERVAL_RANGE_2, AUTH_ERROR_CNT, IOSAPP_REQUEST_LOC_CNT, )
from .const_attrs       import (ZONE, ZONE_DISTANCE, INTERVAL, LAST_UPDATE_TIME,
...
def determine_interval(Device, DeviceFmZone):
    ...
    try:
        ...
        #Region Exited trigger & away & exited less than 1 min ago
        elif (instr(Device.trigger, REGION_EXITED)
                and not_inzone_flag
    ...
    try:
        ...
        event_msg +=(f" > {Zone.display_as} >  "
                    f"NextUpdate-{secs_to_time(next_poll)}, "
                    f"Interval-{interval_str}, "
                    f"TravTime-{waze_time_msg}{dir_of_travel_msg}, "
                    f"Distance-{km_to_mi(dist_from_zone_km)} {Gb.um}, "
...
def determine_interval_after_error(Device, info_msg, counter=OLD_LOC_POOR_GPS_CNT):
    ...
    try:
        ...
        # Often, iCloud does notactuallt locate the device but just returns the last
...
def _get_distance_data(Device, DeviceFmZone):
    ...
    try:
        ...
        #                                         FromZone.latitude,
        #                                         FromZone.longitude)
        calc_dist_from_zone_km = DeviceFmZone.distance
