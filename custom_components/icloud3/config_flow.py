...
#-------------------------------------------------------------------------------------------
    async def async_step_menu(self, user_input=None, errors=None):
        ...
        Gb.HALogger.info(f"BEFORE LIST MENU CHECK {id(self.PyiCloud)=} {self.PyiCloud=} {id(Gb.PyiCloud)=}  {Gb.PyiCloud=} ")
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    def _traceui(self, user_input):
        Gb.HALogger.info(f"{user_input=} {self.errors=} ")
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    async def _log_into_icloud_account(self, user_input, called_from_step_id):
        ...
        Gb.HALogger.info(f"build lists done")
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    async def async_step_device_list(self, user_input=None, errors=None):
        ...
        Gb.HALogger.info(f"BEFORE LIST PYI CHECK {id(self.PyiCloud)=} {self.PyiCloud=} {id(Gb.PyiCloud)=}  {Gb.PyiCloud=} ")
        if self.PyiCloud is None and Gb.PyiCloud is not None:
            self.PyiCloud = Gb.PyiCloud
        Gb.HALogger.info(f"AFTER LIST PYI CHECK {id(self.PyiCloud)=} {self.PyiCloud=} {id(Gb.PyiCloud)=}  {Gb.PyiCloud=} ")
        ...
        if (self.PyiCloud is None
            ...
            Gb.HALogger.info(f"STARTING C-F LOGIN {id(self.PyiCloud)=} {self.PyiCloud=} {id(Gb.PyiCloud)=}  {Gb.PyiCloud=} ")
            await self._log_into_icloud_account({}, self.step_id)

            Gb.HALogger.info(f"AFTER C-F LOGIN {id(self.PyiCloud)=} {self.PyiCloud=} {id(Gb.PyiCloud)=}  {Gb.PyiCloud=} ")
            if (self.PyiCloud and self.PyiCloud.requires_2fa):
                return await self.async_step_reauth(initial_display=True)

        if user_input is None:
            Gb.HALogger.info(f"build lists STARTED")
            await self._build_device_form_selection_lists()
            Gb.HALogger.info(f"build lists done")
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    async def _build_device_form_selection_lists(self):
        Gb.HALogger.info(f"building lists")
...
#----------------------------------------------------------------------
    async def _build_opt_famshr_devices_list(self):
        ...
        # Gb.HALogger.info(f"2127 building famshr list -->return {id(self.PyiCloud)=} {self.PyiCloud=} {id(Gb.PyiCloud)=}  {Gb.PyiCloud=} ")
        if self.PyiCloud is None:
            return
...
#----------------------------------------------------------------------
    def _check_finish_v2v3conversion_for_famshr_fname(self):    #, device_id_by_device_fname,
        ...
        Gb.HALogger.info(f"{famshr_fname_by_ic3_devicename=}")
        ...
        for conf_device in Gb.conf_devices:
            ...
            Gb.HALogger.info(f"Completing FamShr Device assignment > iCloud3 Device-{ic3_devicename}{RARROW}{famshr_fname}")
