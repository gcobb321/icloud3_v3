...
from .const_attrs       import (NAME, FRIENDLY_NAME, LATITUDE, LONGITUDE, ICON,
                                RADIUS, PASSIVE, HIGH_INTEGER, )
from .const_general     import (STATIONARY, STATIONARY_FNAME, HOME,
...
class Zones(object):
    ...
    def __init__(self, zone, zone_data):
        ...
        # Fields set in WazeHist obj
        self.wazehist_db_zone_location_changed_flag = False
        self.wazehist_db_zone_id = 0
    ...
    def distance(self, to_latitude, to_longitude):
        return round(distance(self.latitude, self.longitude,
                            to_latitude, to_longitude), 2)

    # Calculate distance in kilometers
    def distance_km(self, to_latitude, to_longitude):
        return round(distance(self.latitude, self.longitude,
                    to_latitude, to_longitude) / 1000, 2)
...
#           inzone_interval_secs - Configuration parameter
#           still_time - Configuration parameter
#           stationary_zone_offset - Configuration parameter
#        Methods:
...
#
#####################################################################
class StationaryZones(Zones):

    def __init__(self, Device, inzone_interval_secs, still_time, stationary_zone_offset):

        self.zone                      = f"{Device.devicename}_stationary"
        self.Device                    = Device
        self.devicename                = Device.devicename

        # Determine Base Stat Zone location based on Home zone location
        if instr(stationary_zone_offset, "("):
            szo = stationary_zone_offset.replace("(", "").replace(")", "")
            self.base_latitude  =  float(szo.split(',')[0])
            self.base_longitude =  float(szo.split(',')[1])
        else:
            offset_lat          = float(stationary_zone_offset.split(',')[0]) * STATIONARY_ZONE_1KM_LAT
            offset_long         = float(stationary_zone_offset.split(',')[1]) * STATIONARY_ZONE_1KM_LONG

            self.base_latitude  = Gb.HomeZone.latitude  + offset_lat
            self.base_longitude = Gb.HomeZone.longitude + offset_long

        statzone_data = {LATITUDE: self.base_latitude, LONGITUDE: self.base_longitude,
                                RADIUS: 1, PASSIVE: True}

        # Initialize Zone with location
        Zones.__init__(self, self.zone, statzone_data)
        _traceha(f"StatZone/Zone.s__init__-{self.zone=} {statzone_data=}")
        Gb.Zones.append(self)
        Gb.Zones_by_zone[self.zone] = self
        _traceha(f"statzone--init={self.zone=}{Gb.Zones_by_zone=}")
        # Initialize tracking movement fields
        self.inzone_interval_secs      = inzone_interval_secs
        self.still_time                = still_time
        self.stat_zone_half_still_time = still_time / 2
